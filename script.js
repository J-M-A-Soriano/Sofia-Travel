...
function initMap() {
    ...
    sidebar.innerHTML = `
        ...
        <div id="tab-dest" class="map-dest-list" style="display:block; overflow-y:auto;">
            <ul id="mapDestList" style="list-style:none; padding:0; margin:0;"></ul>
            
            <h4 style="padding:15px 15px 5px; margin:0; font-size:0.9rem; color:#64748b; border-top:1px solid #eee; margin-top:10px;">Attractions</h4>
            <ul id="mapAttractionsList" style="list-style:none; padding:0; margin:0;"></ul>
        </div>
...
function updateAttractionsList(destId) {
    // 1. CLEAR THE ROUTE LINE (To prepare for a new one)
    ...
    // 2. Sync the End Point Dropdown (for Route Planner tab)
    ...
    if (data) {
        // We do NOT call mapInstance.flyTo() here anymore.
        // We let flyToDest() in openSpecificMap handle the movement 
        // to avoid double-zooming or conflicts.

        if (data.attractions && data.attractions.length > 0) {
            // Fill Sidebar List with attractions
            data.attractions.forEach(att => {
                const li = document.createElement('li');
                li.className = 'attraction-item';
                li.innerHTML = `<span class="attraction-icon">📷</span> ${att.name}`;
                li.onclick = () => {
                    // Zoom closer to specific attraction
             ...
             list.innerHTML = '<li style="padding:15px; color:#64748b; font-style:italic; text-align:center;">Top attractions coming soon for this location.</li>';
        }
    }
}
...
function openSpecificMap(destId) {
    ...
    // 2. Wait for map to initialize, then Zoom
    setTimeout(() => {
        const data = mapData[destId];
        if (data) {
            ...
            // Execute the Zoom & Route drawing
            flyToDest(destId, data.coords);
            
            // Populate the attractions list (now that the element exists)
            updateAttractionsList(destId);

            // Highlight the list item in sidebar
            document.querySelectorAll('.map-dest-item').forEach(el => el.classList.remove('active'));
            const activeItem = document.getElementById(`dest-item-${destId}`);
            if (activeItem) {
...
/* --- FIXED SUPPORT VIEW (Matches Provided Image) --- */
                ...
                <div>
                    ...
                    <div class="faq-pill">
                        <button class="faq-trigger" onclick="toggleFaq(this)">
                            How do I cancel my booking? <i class="fa-solid fa-chevron-down faq-toggle-icon"></i>
                        </button>
                        <div class="faq-content">
                            You can cancel your booking through the <b>My Bookings</b> section in your dashboard. Select the specific trip and click "Cancel Booking". Refunds are processed according to our cancellation policy.
                        </div>
                    </div>

                    <div class="faq-pill">
                        <button class="faq-trigger" onclick="toggleFaq(this)">
                            Can I reschedule my trip date? <i class="fa-solid fa-chevron-down faq-toggle-icon"></i>
                        </button>
                        <div class="faq-content">
                            Yes, rescheduling is allowed up to 72 hours before your trip date, subject to availability. Please contact our support team or use the "Modify" button in your itinerary.
                        </div>
                    </div>

                    <div class="faq-pill">
                        <button class="faq-trigger" onclick="toggleFaq(this)">
                            When will I receive my refund? <i class="fa-solid fa-chevron-down faq-toggle-icon"></i>
                        </button>
                        <div class="faq-content">
                            Credit card refunds typically take 5-10 business days to reflect on your statement. E-wallet refunds (GCash/Maya) are usually processed within 24-48 hours.
                        </div>
                    </div>

                    <div class="faq-pill">
                        <button class="faq-trigger" onclick="toggleFaq(this)">
                            Is my payment information secure? <i class="fa-solid fa-chevron-down faq-toggle-icon"></i>
                        </button>
                        <div class="faq-content">
                            Absolutely. We use industry-standard 256-bit SSL encryption for all transactions. Your card details are never stored on our servers.
                        </div>
                    </div>

                    <div class="faq-pill">
                        <button class="faq-trigger" onclick="toggleFaq(this)">
                            How do I earn Loyalty Points? <i class="fa-solid fa-chevron-down faq-toggle-icon"></i>
                        </button>
                        <div class="faq-content">
                            Points are automatically added to your account after you complete a booking. You earn 1 point for every ₱500 spent.
                        </div>
                    </div>
                </div>

                <div class="contact-card-floating">
                    <h3 class="contact-card-title">Still need help?</h3>
                    <p class="small muted" style="margin-bottom: 30px;">Our team is available 24/7 to assist you.</p>

                    <div class="contact-option" onclick="toggleChat()" style="cursor:pointer;">
                        <div class="contact-icon-circle"><i class="fa-regular fa-comments"></i></div>
                        <div class="contact-text">
                            <h5>Live Chat</h5>
                            <p>Typically replies in 2 mins</p>
                        </div>
                    </div>
                    ...
                    <h4 style="margin: 0 0 15px 0; color:#1e293b; font-family:'Playfair Display', serif; font-size:1.2rem;">Send a Ticket</h4>
                    <form onsubmit="event.preventDefault(); showToast('Ticket submitted successfully!', 'success'); this.reset();">
                        ...
                        <textarea class="ticket-textarea" rows="3" placeholder="Describe your issue..."></textarea>
